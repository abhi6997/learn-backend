...
const getMyWatchHistory = asyncHandler(async (req, res) => {
  const myWatchHistoryData = await User.aggregate([
    {
      $match: {
    ...
    {$lookup:{
      ...
      {
        $addFields:{"watchHistory.owner":{$first:"$watchHistory.owner"}}   //array -> object
      },
  ...
  return res
  .status(200)
  .json(new ApiResponse(200,myWatchHistoryData,"watch history successfully fetched"))
});
